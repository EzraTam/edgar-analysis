...
def norm_and_rename(method):
    ...
    def inner(self):
        ...
        if self.norm_mill:
            df_output["val"] = df_output["val"] / 1_000_000
        if self.res_col_nm:
...
class EdgarData:
    ...
    def __init__(
        ...
        res_col_nm: Optional[str] = None,
        norm_mill: Optional[bool] = None,
    ) -> None:
    ...
    def from_tag(
        ...
        assert (
            which in list_which
        ), f"Please choose which only from the following list: {list(list_which)}"

        data_to_extract = data_comp["facts"][which]

        assert (
            len(data_to_extract[tag]["units"]) <= 1
        ), f"There are more than 1 data with different units! Units are: {list(data_to_extract[tag]['units'].keys())}"

        assert (
            len(data_to_extract[tag]["units"]) > 0
        ), "No data is given or data is directly given in unit. Please check"
        ...
        return EdgarData(
            data=data_to_extract[tag]["units"][unit],
            unit=unit,
    ...
    def raw(self) -> pd.DataFrame:
        ...
        _df["year"] = pd.DatetimeIndex(_df["end"]).year # pylint: disable=E1101
    ...
    def quarterly(self) -> pd.DataFrame:
        ...
        df_temp = df_temp.drop(columns=["sum", "count", "val"]).rename(
            columns={"Q4": "val"}
        )
        df_temp["fp"] = "Q4"
        ...
        df_quarter = df_quarter.sort_values(
            by=["year", "fp"], ascending=[False, False]
        ).reset_index(drop=True)

        return (
            df_quarter.sort_values(by=["year", "fp"], ascending=[False, False])
            .reset_index(drop=True)
            .rename(columns={"fp": "quarter"})
        )
...
class EdgarDataComp:
    ...
    def by_tag(
        self, tag: str, which: str, res_col_nm: Optional[str] = None
    ) -> pd.DataFrame:
        ...
        # List tags for which no need for normalization
        tags_no_norm = ["EarningsPerShareDiluted"]

        norm_mill = False if tag in tags_no_norm else self.norm_mill
    ...
    def shares_outstanding(self) -> pd.DataFrame:
        ...
        _df_shares = pd.DataFrame(
            self.data_comp["facts"]["dei"]["EntityCommonStockSharesOutstanding"][
                "units"
            ]["shares"]
        )[["fy", "fp", "val"]]
        _df_shares = _df_shares.rename(columns={"fy": "year", "fp": "quarter"})
        if self.norm_mill:
            _df_shares["val"] = _df_shares["val"] / 1_000_000
        ...
        return _df_shares.sort_values(
            ["year", "quarter"], ascending=[False, False], ignore_index=True
        )
    ...
    def market_capitalization(self) -> pd.DataFrame:
        ...
        _df_mc = pd.DataFrame(
            self.data_comp["facts"]["dei"]["EntityPublicFloat"]["units"]["USD"]
        )[["fy", "val"]].rename(columns={"fy": "year"})
        if self.norm_mill:
    ...
    def avg_share_price(self) -> pd.DataFrame:
        ...
        _df_res = pd.merge(
            self.shares_outstanding.rename(columns={"val": "so"}),
            self.market_capitalization.rename(columns={"val": "mc"}),
            on="year",
        )
        _df_res["val"] = _df_res["mc"] / _df_res["so"]

        self.interval = interval_old
        return _df_res.drop(columns=["mc", "so"]).sort_values("year", ascending=False, ignore_index=True)

    @property
    def avg_p_e(self) -> pd.DataFrame:
        """ Average PE
        """
        interval_old = self.interval
        self.interval = "yearly"
        _df_res= pd.merge(
            self.by_tag(tag="EarningsPerShareDiluted",which="us-gaap").rename(columns={"val": "eps"}),
            self.avg_share_price.rename(columns={"val": "sp"}),
            on="year",
        )
        _df_res["val"] = _df_res["sp"] / _df_res["eps"]
        self.interval = interval_old
        return _df_res.drop(columns=["sp", "eps"]).sort_values("year", ascending=False, ignore_index=True)
